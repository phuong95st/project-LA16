...
<c:if test="${listTeachInfo!= null && listTeachInfo.size() != 0 }">
		...
		<table class="table table-condensed table-bordered small">
			...
			<c:forEach items="${listTeachInfo}" var="teachInfo">
				<tr>
					...
					<td>${teachInfo.teach.weekStart.weekCount }-${teachInfo.teach.weekEnd.weekCount }</td>
					<td>${teachInfo.teach.weekEnd.hocKy.name }</td>
					<td>${teachInfo.teach.codeClass }</td>
					<td>${teachInfo.teach.codeSubject }</td>
					<td>${teachInfo.teach.name }</td>
					<td>${teachInfo.teach.user.name }</td>
					<td><c:choose>
							<c:when test="${teachInfo.hol }">
								<span class="text-danger">Không dạy</span>
							</c:when>
							<c:when test="${teachInfo.late }">
								<span class="text-warning"> Đến muộn <b>${teachInfo.lateMin}</b>
									phút. Lý do: ${teachInfo.reason }
								</span>
							</c:when>
							<c:otherwise>
								<span class="text-success">Có dạy</span>
							</c:otherwise>
						</c:choose></td>
				</tr>
...
<c:if test="${lisScheStus != null && lisScheStus.size() != 0 }">
...
<c:if test="${lisOnls != null && lisOnls.size() != 0 }">
		...
		<table class="table table-condensed table-bordered">
			...
			<c:forEach items="${lisOnls}" var="onl">
				<tr>
					...
					<td><c:choose>
							<c:when test="${onl.hol }">
								<span class="text-danger">Không trực</span>
							</c:when>
							<c:when test="${onl.late }">
								<span class="text-warning"> Trực muộn <b>${onl.lateMin}</b>
									phút. Lý do: "${onl.reason }"
								</span>
							</c:when>
							<c:otherwise>
								<span class="text-success">Trực đúng giờ</span>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
...
<c:if test="${listTeachInfo.size() == 0 && lisScheStus.size() == 0 && lisOnls.size() == 0}">
