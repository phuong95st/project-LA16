...
//				int grpId = user.getGroupId();
				...
				if("full_name".equals(sortType)){
					sqlCommand.append(" ORDER BY "+sortType+" "+ sortByFullName +", j.code_level DESC, end_date DESC");
				}else if("code_level".equals(sortType)){
					sqlCommand.append(" ORDER BY j."+sortType+" "+ sortByFullName+", full_name ASC, end_date DESC");
				}else{
					sqlCommand.append(" ORDER BY "+sortType+" "+ sortByFullName+", full_name ASC, j.code_level DESC");
				}
